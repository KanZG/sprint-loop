...
> **スキーマ準拠**: 全フィールド名は `snake_case`。`current_sprint` は数値 `1`（`"sprint-001"` ではない）。`completed_review_axes` は空配列 `[]`。`phase` は `"executing"`。
...
> **スキーマ準拠**: 全フィールド名は `snake_case`。`resume_mode`, `previous_subphase`, `dod_retry_count`, `completed_review_axes` の名前を正確に。
